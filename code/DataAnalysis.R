...
analyze_variable_two <- function(
  ...
  safe_analyze <- safely(function() {
    ...
    for (factor in random_factors) {
      grouping <- droplevels(as.factor(data_clean[[factor]]))
      levels <- nlevels(grouping)
      repeated <- any(table(grouping) > 1)
      if (levels > 1 && levels < nrow(data_clean) && repeated) {
        valid_random_effects <- c(valid_random_effects, paste("(1 |", factor, ")"))}}
...
dataOrigin <- read.xlsx(field_file, "dataall(plain)") %>% mutate(
  ...
  PlotID = interaction(Site, Crop, Treat_All, Block, drop = TRUE, lex.order = TRUE),
