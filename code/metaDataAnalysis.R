...
# custom function ------------------------------------------------------------------
# Nakagawa S, Noble D W A, Lagisz M, Spake R, Viechtbauer W, Senior A M. A robust and readily implementable method for the meta-analysis of response ratios with and without missing standard deviations[J]. Ecology Letters, 2023, 26(2): 232-244. https://doi.org/10.1111/ele.14144
#' @title cv_avg
...
#' @title lnrr_laj
#' @description Calculates log response ratio based on Taylor expansion from Lajeunesse 2015
#' @param m1 Mean of group 1 (control)
#' @param m2 Mean of group 2 (treatment)
#' @param cv1_2 Coefficient of variation squared (CV^2) for group 1
#' @param cv2_2 Coefficient of variation squared (CV^2) for group 2
...
#' @title v_lnrr_laj
#' @description Calculates the sampling variance for log response ratio based on second order Taylor expansion proposed by Lajeunesse 2015
#' @param cv1_2 Coefficient of variation squared (CV^2) for group 1
#' @param cv2_2 Coefficient of variation squared (CV^2) for group 2
#' @param n1 Sample size for group 1 (control)
#' @param n2 Sample size for group 2 (treatment)
#' @param taylor A logical indicating whether to calculate point estimate with Taylor expansion.
...
# Lajeunesse M J. On the meta-analysis of response ratios for studies with correlated and multi-group designs[J]. Ecology, 2011, 92(11): 2049-2055. https://doi.org/10.1890/11-0423.1
...
rm(list = ls()) # clear the environment
cat("\014") # clear the console
pacman::p_load(openxlsx, tidyverse, metafor)

data_combined_wide3 <- read.xlsx("Input/metaData.xlsx", "data_combined_wide") %>%
...
write.xlsx(data_combined_with_con5, "output/data_combined_with_con.xlsx", rowNames = FALSE)

## Calculation of pooled effect size -----------------------------------------------------------------
data_combined_with_con5 <- read.xlsx("output/data_combined_with_con.xlsx", "Sheet 1")
...
model_RootLength_ess.se <- rma.mv(
  ...
  mods = ~ ess.se, test = "t", dfs = "contain", control = list(optimizer = "Nelder-Mead", maxit = 10000))
#### Yield
Yield_data <- data_plant %>% filter(VariableNew == "Yield", Irrigation %in% c("No"))
...
model_Yield <- rma.mv(
  ...
  random = list(~ 1 | ExpID, ~ 1 | SameVarIDN),
  test = "t", dfs = "contain", # verbose = TRUE,
...
cairo_pdf("Output/ES-Soil.pdf", bg = "transparent", width = 30/2.54, height = 10/2.54)
p_ES_Soil
dev.off()

### composite Index ----------------------------------------------------------------------
# some indicators are usually "the smaller the better", so negative values are used to unify the direction.
# some indicators (such as pH). usually have the best values in the middle and the worst at the extremes. not be included in the calculation of the comprehensive index based on monotonic directionality.
neg_vars <- c("BD", "CAT", "POX", "PER", "OX", "BFratio", "nirK", "nirS", "N2O")
data_combined_with_con7 <- data_combined_with_con6 %>%
  mutate(yi = if_else(Variable %in% neg_vars, -yi, yi)) %>%
  filter(VariableNew != "pH")
data_PGI <- data_combined_with_con7 %>% filter(Niche == "plant", Irrigation %in% c("No"))
data_SQI <- data_combined_with_con7 %>%
  filter(Niche != "plant" | is.na(PlantAppear)) %>% filter(Irrigation %in% c("No"))
data_EMF <- data_combined_with_con7 %>% filter(Irrigation %in% c("No"))
#### PGI
...
ggplot(new1, aes(x = estimate, y = model)) +
  ...
  geom_text(aes(label = paste0("(", n, ")")), x = max(abs(c(new1$ci.lb, new1$ci.ub))) * 1.2,
  ...
  coord_cartesian(xlim = c(-max(abs(c(new1$ci.lb, new1$ci.ub))) * 1.4, max(abs(c(new1$ci.lb, new1$ci.ub))) * 1.4)) +
  theme(aspect.ratio = 3.5/2) -> p_ES_Index; p_ES_Index
pdf("Output/ES-Index.pdf",
    height = 12/2.54)
p_ES_Index
dev.off()
...
ggplot(new2, aes(x = estimate, y = model)) +
  ...
  geom_text(aes(label = paste0("(", n, ")")), x = max(abs(c(new2$ci.lb, new2$ci.ub))) * 1.4*0.995,
            hjust = 1, size = 12/2.8346, color = "black") +
  ...
  labs(x = "Effect Size", y = NULL) + theme_classic() + theme(
    ...
    axis.title = element_text(color = "black", size = 15),
    plot.margin = margin(0, 0, 0, 0)) +
  coord_cartesian(xlim = c(-max(abs(c(new2$ci.lb, new2$ci.ub))) * 1.4, max(abs(c(new2$ci.lb, new2$ci.ub))) * 1.4)) +
  theme(aspect.ratio = 2.5/2) -> p_Plant_Index; p_Plant_Index
cairo_pdf("Output/ES-Plant.pdf", bg = "transparent", width = 30/2.54, height = 10/2.54)
p_Plant_Index
dev.off()

## Visualization of publication bias ------------------------------------------------------------------
...
batch_generate_funnel <- function(indicator_list, suffix = NULL) {
  ...
  for (indicator in indicator_list) {
    ...
    if (!exists(model_name) || !exists(ess.se_name)) {
      warning(paste("⚠️ ", model_name, " or ", ess.se_name, " not exist. skip ", indicator))
      next}
...
batch_generate_funnel(my_indicators, suffix = "")

## Sensitivity analysis --------------------------------------------------------------
pacman::p_load(metafor, future.apply)
data_ExpID <- read.xlsx("Input/metaData.xlsx", "basic")
Yield_data$`Exp. ID` <- NA
Yield_data$`Exp. ID` <- data_ExpID$`Exp..ID`[match(Yield_data$ExpID, data_ExpID$`ExpID`)]
data_SQI$`Exp. ID` <- NA
data_SQI$`Exp. ID` <- data_ExpID$`Exp..ID`[match(data_SQI$ExpID, data_ExpID$`ExpID`)]
data_EMF$`Exp. ID` <- NA
data_EMF$`Exp. ID` <- data_ExpID$`Exp..ID`[match(data_EMF$ExpID, data_ExpID$`ExpID`)]
...
cairo_pdf("Output/LOO-Yield.pdf", bg = "transparent", width = 30/2.54, height = 15/2.54)
p_LOO_Yield
dev.off()
...
cairo_pdf("Output/LOO-SQI.pdf", bg = "transparent", width = 30/2.54, height = 15/2.54)
p_LOO_SQI
dev.off()
...
cairo_pdf("Output/LOO-EMF.pdf", bg = "transparent", width = 30/2.54, height = 15/2.54)
p_LOO_EMF
dev.off()
...
rm(list = ls()) # clear the environment
cat("\014") # clear the console
pacman::p_load(openxlsx, tidyverse, metafor, metaforest, glmulti, MuMIn, future.apply)
...
neg_vars <- c("BD", "CAT", "POX", "PER", "OX", "BFratio", "nirK", "nirS", "N2O")
data_combined_with_con7 <- data_combined_with_con6 %>%
  mutate(yi = if_else(Variable %in% neg_vars, -yi, yi)) %>% filter(VariableNew != "pH")
data_SQI <- data_combined_with_con7 %>%
  filter(Niche != "plant" | is.na(PlantAppear)) %>% filter(Irrigation %in% c("No"))
data_EMF <- data_combined_with_con7 %>% filter(Irrigation %in% c("No"))
...
check_vars_Yield <-
  c("SampleTime2", "Niche", "SoilDepth", "PlantAppear",
    ...
    "Ecosystem", "AridityIndex", "MAT", "MAP", "FAO90", "AWC", "TEXTURE_USDA",
    "BULK", "ORG_CARBON", "PH_WATER", "TOTAL_N", "ELEC_COND")
valid_vars_Yield <- c()
...
cairo_pdf("Output/VarImp-Yield.pdf", bg = "transparent", width = 30/2.54, height = 6.7/2.54)
p_VarImp_Yield
dev.off()
...
full_importance_SQI$Variable_New <- c("Type", "Dose", "Duration", "MAT", "MAP", "Niche")
...
cairo_pdf("Output/VarImp-SQI.pdf", bg = "transparent", width = 30/2.54, height = 6.7/2.54)
p_VarImp_SQI
dev.off()
...
cairo_pdf("Output/VarImp-EMF.pdf", bg = "transparent", width = 30/2.54, height = 6.7/2.54)
p_VarImp_EMF
dev.off()
...
pacman::p_load(openxlsx, tidyverse, metafor, future.apply, patchwork, orchaRd)
...
df_Yield_PlasType <- extract_model_results(
  model_Yield_PlasType, c("BioMix", "PE", "PLA", "PP"))
...
cairo_pdf("Output/Sub-Yield.pdf", bg = "transparent", width = 10.85/2.54, height = 30/2.54)
p_Sub_Yield
dev.off()
...
cairo_pdf("Output/Indi-Yield.pdf", bg = "transparent", width = 8.88/2.54, height = 30/2.54)
p_Indi_Yield
dev.off()
...
result_Sub_Yield_PlasType <- result_Sub_Yield_PlasType %>% mutate(Group1 = factor(c("PE", "PP", "PLA", "BioMix")))
...
cairo_pdf("Output/Sub-Yield-PlasType.pdf", bg = "transparent", width = 30/2.54, height = 12/2.54)
...
cairo_pdf("Output/Sub-Yield-PlasDose.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
cairo_pdf("Output/Sub-Yield-SampleTime2.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
cairo_pdf("Output/Sub-Yield-MAT.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_Yield_MAT
dev.off()
...
cairo_pdf("Output/Sub-Yield-PlasSize.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_Yield_PlasSize
dev.off()
cairo_pdf("Output/Sub-Yield-PlasSize2.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
cairo_pdf("Output/Sub-Yield-MAP.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_Yield_MAP
dev.off()
...
cairo_pdf("Output/Sub-Yield-AridityIndex.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
cairo_pdf("Output/Sub-SQI.pdf", bg = "transparent", width = 10.85/2.54, height = 30/2.54)
p_Sub_SQI
dev.off()
...
cairo_pdf("Output/Indi-SQI.pdf", bg = "transparent", width = 8.88/2.54, height = 30/2.54)
p_Indi_SQI
dev.off()
...
cairo_pdf("Output/Sub-SQI-PlasType.pdf", bg = "transparent", width = 30/2.54, height = 12/2.54)
p_Sub_SQI_PlasType
dev.off()
...
cairo_pdf("Output/Sub-SQI-PlasDose.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_SQI_PlasDose
dev.off()
...
cairo_pdf("Output/Sub-SQI-SampleTime2.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
cairo_pdf("Output/Sub-SQI-MAT.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_SQI_MAT
dev.off()
...
cairo_pdf("Output/Sub-SQI-PlasSize.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_SQI_PlasSize
dev.off()
cairo_pdf("Output/Sub-SQI-PlasSize2.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_SQI_PlasSize2
dev.off()
...
cairo_pdf("Output/Sub-SQI-MAP.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_SQI_MAP
dev.off()
...
cairo_pdf("Output/Sub-SQI-AridityIndex.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
end_EMF_Sub - start_EMF_Sub # ~ 2.7 h
plan(sequential)
...
end_EMF_Sub2 - start_EMF_Sub2 # ~ 1.3 h
plan(sequential)
...
cairo_pdf("Output/Sub-EMF.pdf", bg = "transparent", width = 10.85/2.54, height = 30/2.54)
p_Sub_EMF
dev.off()
...
cairo_pdf("Output/Indi-EMF.pdf", bg = "transparent", width = 8.88/2.54, height = 30/2.54)
p_Indi_EMF
dev.off()
...
cairo_pdf("Output/Sub-EMF-PlasType.pdf", bg = "transparent", width = 30/2.54, height = 12/2.54)
p_Sub_EMF_PlasType
dev.off()
...
cairo_pdf("Output/Sub-EMF-PlasDose.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_EMF_PlasDose
dev.off()
...
cairo_pdf("Output/Sub-EMF-SampleTime2.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
cairo_pdf("Output/Sub-EMF-MAT.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_EMF_MAT
dev.off()
...
cairo_pdf("Output/Sub-EMF-PlasSize.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_EMF_PlasSize
dev.off()
cairo_pdf("Output/Sub-EMF-PlasSize2.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_EMF_PlasSize2
dev.off()
...
cairo_pdf("Output/Sub-EMF-MAP.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
p_Sub_EMF_MAP
dev.off()
...
cairo_pdf("Output/Sub-EMF-AridityIndex.pdf", bg = "transparent", height = 5.3/2.54, width = 30/2.54)
...
QM_results_PGI <- bind_rows(
  extract_mod_test(model_PGI_PlasType2, "PlasType2"),
  extract_mod_test(model_PGI_PlasSize2, "PlasSize2"),
  extract_mod_test(model_PGI_PlasDose2, "PlasDose2"),
  extract_mod_test(model_PGI_PlasShape, "PlasShape"),
  extract_mod_test(model_PGI_PlasType, "PlasType"),
  extract_mod_test(model_PGI_PlasDose, "PlasDose"),
  extract_mod_test(model_PGI_PlasSize, "PlasSize"),
  extract_mod_test(model_PGI_SampleTime2, "SampleTime2"),
  extract_mod_test(model_PGI_MAT, "MAT"),
  extract_mod_test(model_PGI_MAP, "MAP"),
  extract_mod_test(model_PGI_AridityIndex, "AridityIndex"))
...
## Map Location ------------------------------------------------------------
pacman::p_load(openxlsx, tidyverse, terra, tidyterra, ggnewscale)
BaseMap <- rast("D:/ArcGIS/GlobalAIPET/ai_v31_yr.tif") / 10000
BaseMap_admin <- classify(BaseMap, cbind(0, NA))
points_df <- read.xlsx("Input/metaData.xlsx", "Points_with_Climate_Elevation") %>%
  filter(Irrigation == "No")
points_df$location <- paste(points_df$Longitude, points_df$Latitude, sep = "_")
point_count <- points_df %>%
  group_by(Ecosystem, Group1, location = tolower(location)) %>%
...
point_literature <- point_literature %>%
  mutate(Count_size = case_when(Count == 1 ~ "1", Count == 2 ~ "2", Count >= 3 ~ "3+") %>% factor(levels = c("1", "2", "3+")),
  Ecosystem = factor(Ecosystem, levels = c("Agricultural", "Natural"))) %>% arrange(desc(Ecosystem))
ggplot() +
  geom_spatraster(data = BaseMap_admin, maxcell = 2e7) +
  ...
  geom_point(data = point_literature, color = "black", stroke = 0.8,
             shape = 21,
             aes(x = Longitude, y = Latitude, fill = Ecosystem, size = Count_size)) +
...
pdf("Output/MapLocation.pdf", width = 29/2.54)
p_Map2
dev.off()
...
points <- read.xlsx("Input/metaData.xlsx", "Points_with_Climate_Elevation") %>%
  filter(Irrigation == "No")
...
whittaker_base_plot() +
  geom_point(data = points_literature,
             aes(x = Annual_Mean_Temp, y = Annual_Precip/10), shape = 21,
